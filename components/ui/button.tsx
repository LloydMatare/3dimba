...
function Button({
  ...
  if (asChild) {
    ...
    // Base UI button semantics default to expecting a native <button>. When we
    // "asChild" render something else (e.g. <a>), we must opt out.
    const inferredNativeButton =
      typeof children.type === "string" ? children.type === "button" : undefined
    const nativeButton =
      props.nativeButton ?? inferredNativeButton ?? false
